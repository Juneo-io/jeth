...
Avalanche Warp Messaging offers a basic primitive to enable Cross-Supernet communication on the Avalanche Network.

It is intended to allow communication between arbitrary Custom Virtual Machines (including, but not limited to Supernet-EVM and Coreth).
...
Every node tracking a Supernet has read access to the Avalanche P-Chain. This provides weighted sets of BLS Public Keys that correspond to the validator sets of each Supernet on the Avalanche Network. Avalanche Warp Messaging provides a basic primitive for signing and verifying messages between Supernets: the receiving network can verify whether an aggregation of signatures from a set of source Supernet validators represents a threshold of stake large enough for the receiving network to process the message.
...
### Performance Optimization: C-Chain to Supernet

To support C-Chain to Supernet communication, or more generally Primary Network to Supernet communication, we special case the C-Chain for two reasons:

1. Every Supernet validator validates the C-Chain
2. The Primary Network has the largest possible number of validators

Since the Primary Network has the largest possible number of validators for any Supernet on Avalanche, it would also be the most expensive Supernet to receive and verify Avalanche Warp Messages from as it reaching a threshold of stake on the primary network would require many signatures. Luckily, we can do something much smarter.

When a Supernet receives a message from a blockchain on the Primary Network, we use the validator set of the receiving Supernet instead of the entire network when validating the message. This means that the C-Chain sending a message can be the exact same as Supernet to Supernet communication.

However, when Supernet B receives a message from the C-Chain, it changes the semantics to the following:

1. Read the SourceChainID of the signed message (C-Chain)
2. Look up the SupernetID that validates C-Chain: Primary Network
3. Look up the validator set of Supernet B (instead of the Primary Network) and the registered BLS Public Keys of Supernet B at the P-Chain height specified by the ProposerVM header
4. Continue Warp Message verification using the validator set of Supernet B instead of the Primary Network

This means that C-Chain to Supernet communication only requires a threshold of stake on the receiving supernet to sign the message instead of a threshold of stake for the entire Primary Network.

This assumes that the security of Supernet B already depends on the validators of Supernet B to behave virtuously. Therefore, requiring a threshold of stake from the receiving Supernet's validator set instead of the whole Primary Network does not meaningfully change security of the receiving Supernet.
...
Verifying a message requires looking up the validator set of the source supernet on the P-Chain. To support this, Avalanche Warp Messaging uses the ProposerVM header, which includes the P-Chain height it was issued at as the canonical point to lookup the source supernet's validator set.
