...
func (vm *VM) Initialize(
	...
	log.Info("Initializing JEth VM", "Version", Version, "Config", vm.config)
...
func (vm *VM) preBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	for {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err = tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.ChainAssetID, header.BaseFee)
			if err != nil {
...
func (vm *VM) postBatchOnFinalizeAndAssemble(header *types.Header, state *state.StateDB, txs []*types.Transaction) ([]byte, *big.Int, *big.Int, error) {
	...
	for {
		...
		// ApricotPhase5.
		txContribution, txGasUsed, err = tx.BlockFeeContribution(true, vm.ctx.ChainAssetID, header.BaseFee)
		if err != nil {
			return nil, nil, nil, err
		}
...
func (vm *VM) onExtraStateChange(block *types.Block, state *state.StateDB) (*big.Int, *big.Int, error) {
	...
	for _, tx := range txs {
		...
		if rules.IsApricotPhase4 {
			contribution, gasUsed, err := tx.BlockFeeContribution(rules.IsApricotPhase5, vm.ctx.ChainAssetID, block.BaseFee())
			if err != nil {
				return nil, nil, err
			}
...
func (vm *VM) estimateBaseFee(ctx context.Context) (*big.Int, error) {
	...
	if baseFee == nil {
		baseFee = vm.chainConfig.GetCurrentBaseFee(vm.eth.APIBackend.LastAcceptedBlock().Time())
	} else {
