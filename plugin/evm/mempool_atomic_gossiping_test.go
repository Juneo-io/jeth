...
func TestMempoolPriorityDrop(t *testing.T) {
	...
	tx1, err := vm.newImportTx(vm.ctx.JVMChainID, testEthAddrs[0], initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
	if err != nil {
		t.Fatal(err)
	}
	...
	tx2, err := vm.newImportTx(vm.ctx.JVMChainID, testEthAddrs[1], initialBaseFee, []*secp256k1.PrivateKey{testKeys[1]})
	if err != nil {
		t.Fatal(err)
	}
	...
	tx3, err := vm.newImportTx(vm.ctx.JVMChainID, testEthAddrs[1], new(big.Int).Mul(initialBaseFee, big.NewInt(2)), []*secp256k1.PrivateKey{testKeys[1]})
	if err != nil {
		t.Fatal(err)
	}
